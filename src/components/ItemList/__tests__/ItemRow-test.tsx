...
describe('components/Popups/ReplyField/ItemRow', () => {
    ...
    describe('render()', () => {
        test('should render item row with correct props', () => {
            const wrapper = getWrapper({ isActive: true });

            expect(wrapper.find('[data-testid="ba-ItemRow"]').prop('className')).toEqual('ba-ItemRow is-active');
        });

        test('should not render anything if no item', () => {
            const wrapper = getWrapper({ item: null });

            expect(wrapper.exists('[data-testid="ba-ItemRow"]')).toBeFalsy();
        });
